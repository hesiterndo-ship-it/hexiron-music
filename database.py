...
def init_db():
    ...
    with _lock, get_conn() as conn:
        conn.executescript(
            ...
            CREATE INDEX IF NOT EXISTS idx_queue_chat ON queue(chat_id, sort_order);
...
def add_to_queue(
    ...
    with _lock, get_conn() as conn:
        max_order = conn.execute(
            "SELECT COALESCE(MAX(sort_order), 0) FROM queue WHERE chat_id=?",
            (chat_id,),
        ).fetchone()[0]

        cur = conn.execute(
            "INSERT INTO queue(chat_id, title, artist, duration, file_path, source, "
            "requested_by, added_at, sort_order) "
            "VALUES (?, ?, ?, ?, ?, ?, ?, datetime('now'), ?)",
            (chat_id, title, artist, duration, file_path, source, requested_by, max_order + 1),
        )
        conn.commit()
...
def pop_next(chat_id: int) -> Optional[Dict]:
    ...
    with _lock, get_conn() as conn:
        row = conn.execute(
            ...
            "FROM queue WHERE chat_id=? ORDER BY sort_order ASC LIMIT 1",
            (chat_id,),
        ).fetchone()
        if row is None:
            return None
...
def peek_queue(chat_id: int) -> List[Dict]:
    with _lock, get_conn() as conn:
        rows = conn.execute(
            "SELECT id, title, artist, duration, file_path, source, requested_by, added_at "
...
def remove_queue_item(chat_id: int, item_id: int) -> bool:
    with _lock, get_conn() as conn:
        cur = conn.execute(
            "DELETE FROM queue WHERE id=? AND chat_id=?", (item_id, chat_id)
        )
        conn.commit()
...
def shuffle_queue(chat_id: int):
    ...
    with _lock, get_conn() as conn:
        ...
        for idx, qid in enumerate(ids):
            conn.execute(
                "UPDATE queue SET sort_order=? WHERE id=?", (idx, qid)
            )
        conn.commit()


def move_queue_item(chat_id: int, item_id: int, new_position: int) -> bool:
...
def add_favorite(
    ...
    with _lock, get_conn() as conn:
        try:
            conn.execute(
                "INSERT INTO favorites(user_id, title, artist, duration, file_path, source, added_at) "
                "VALUES (?, ?, ?, ?, ?, ?, datetime('now'))",
...
def remove_favorite(user_id: int, title: str, artist: str = "") -> bool:
    with _lock, get_conn() as conn:
        cur = conn.execute(
            "DELETE FROM favorites WHERE user_id=? AND title=? AND artist=?",
...
def get_favorites(user_id: int, limit: int = 50, offset: int = 0) -> List[Dict]:
    with _lock, get_conn() as conn:
        rows = conn.execute(
...
def is_favorite(user_id: int, title: str, artist: str = "") -> bool:
    with _lock, get_conn() as conn:
        row = conn.execute(
            "SELECT 1 FROM favorites WHERE user_id=? AND title=? AND artist=?",
            (user_id, title, artist),
        ).fetchone()
...
def update_chat_setting(chat_id: int, key: str, value):
    ...
    with _lock, get_conn() as conn:
        conn.execute(
            f"UPDATE chat_settings SET {key}=?, updated_at=datetime('now') WHERE chat_id=?",
            (value, chat_id),
        )
...
def get_stat(metric: str) -> int:
    with _lock, get_conn() as conn:
        row = conn.execute(
            "SELECT value FROM statistics WHERE metric=?", (metric,)
        ).fetchone()
        return row[0] if row else 0
...
def get_all_stats() -> Dict[str, int]:
    with _lock, get_conn() as conn:
        rows = conn.execute("SELECT metric, value FROM statistics").fetchall()
